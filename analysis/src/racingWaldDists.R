...
dWald <- function(t,v,B,A, useSuppDists=TRUE)
...
rWaldRaceSM <- function(n, A, B, t0, v, st0 = 0, silent = FALSE, args.dist=NULL, simPerTrial=FALSE)
{
  if(is.list(v)) {
    ...
    if(simPerTrial) {
      for(i in 1:nrow(v)) {
        out[i,] <- rWaldRace(n=n, v=v[i,], B=B[i], A=A, t0=t0)
      }    
    } else {
      ...
      for(i in 1:n_v) {
        ttf[i,] <- t0 + rinvGauss(n, nu=bs/v[,i], lambda=bs^2)
      }
    ...
    out <- rWaldRace(n=n, v=v, B=B, A=A, t0=t0)
  }
  out
}
