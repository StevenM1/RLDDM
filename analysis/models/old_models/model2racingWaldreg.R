...
modelSpec = list('variablePars'=list('B' = 'condition',
                                     ...
                                     'b2' = 1,
                                     'b3' = 1),
                 'constants'=c('st0'=0, 'A'=0, 'eta2'=-Inf, 'startingValue'=0.00001),
                 'condition'=c('SPD', 'ACC'),
                 'learningRule'='SARSA')
...
transformChoicePars <- function(pars, condition, ev) {
  ...
  # all models assume a linear relation between ev and v
  # v1 = ev[,1]*pars[['m']]
  # v2 = ev[,2]*pars[['m']]
  
  v1 = pars[['b0']] + pars[['b1']]*ev[,1] + pars[['b2']]*(ev[,1]+ev[,2]) + pars[['b3']]*ev[,2]
  v2 = pars[['b0']] + pars[['b1']]*ev[,2] + pars[['b2']]*(ev[,1]+ev[,2]) + pars[['b3']]*ev[,1]  
